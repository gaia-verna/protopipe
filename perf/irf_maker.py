...
class SensitivityMaker(object):
    ...
    def estimate_sensitivity(self):
        ...
        bkg_syst = self.config["analysis"]["bkg_syst"]
        
        sensitivity_estimator = SensitivityEstimator(irf=self.irf, livetime=obs_time, slope=slope, alpha=alpha, sigma=sigma_min, gamma_min=gamma_min, bkg_sys=bkg_syst)
...
class IrfMaker(object):
    ...
    def make_bkg_rate(self):
        ...
        for ibin, info in enumerate(self.table):
            ...
            n_p = sum(
                data_p[
                    (data_p[colname_reco_energy] >= info["emin"])
                    & (data_p[colname_reco_energy] < info["emax"])
            ...
            n_e = sum(
                data_e[
                    (data_e[colname_reco_energy] >= info["emin"])
                    & (data_e[colname_reco_energy] < info["emax"])
        ...
        t["BGD"] = Column(bgd, unit="Hz", description="Background", format="E")
    ...
    def make_point_spread_function(self, radius=68):
        ...
        for ibin, info in enumerate(self.table):
            ...
            sel = data_g.loc[
                (data_g[colname_reco_energy] >= info["emin"])
                & (data_g[colname_reco_energy] < info["emax"])
                & (data_g["pass_best_cutoff"]),
                [colname_angular_dist],
            ]

            # Compute PSF
            psf[ibin] = np.percentile(sel[colname_angular_dist], radius)
        ...
        t["PSF68"] = Column(psf, unit="deg", description="PSF", format="E")
    ...
    def make_effective_area(
        ...
        colname_mc_energy = self.config['column_definition']['mc_energy']
        ...
        for ibin in range(nbin):
            ...
            condition = (data_g[colname_mc_energy] >= emin) & (data_g[colname_mc_energy] < emax)
    ...
    def make_energy_dispersion(self):
        ...
        for imigra in range(len(migra) - 1):
            ...
            for ietrue in range(len(etrue) - 1):
                ...
                sel = len(
                    data_g[
                        (data_g[colname_mc_energy] >= emin)
                        & (data_g[colname_mc_energy] < emax)
                        & ((data_g[colname_reco_energy] / data_g[colname_mc_energy]) >= migra_min)
                        & ((data_g[colname_reco_energy] / data_g[colname_mc_energy]) < migra_max)
                    ]
                )
