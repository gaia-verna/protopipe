...
from protopipe.perf import CutsOptimisation, CutsDiagnostic, CutsApplicator, IrfMaker, SensitivityMaker
...
def main():
    ...
    for particle in particles:
        ...
        evt_dict[particle]['index'] = evt_dict[particle].index
        
    n_boostrap = 100
    evt_dict_sample = dict()
    ...
    for i in range(n_boostrap):
        ...
        outdir = os.path.join(cfg['general']['outdir'], 'irf_{}_ThSq_{}_Time{:.2f}{}_{}'.format(
            args.mode,
        ...
        for particle in particles:
            evt_dict_sample[particle] = evt_dict[particle].loc[evt_dict[particle]['index'].isin(np.random.choice(len(evt_dict[particle]['index']),len(evt_dict[particle]['index'])))]
        ...
        for particle in ['electron', 'proton']:
            ...
            evt_dict_sample[particle] = evt_dict_sample[particle].query('offset <= {}'.format(
        ...
        for particle in particles:
            cfg['particle_information'][particle]['n_files'] = \
                len(np.unique(evt_dict_sample[particle]['obs_id']))
        ...
        if thsq_opt_type in 'fixed':
            ...
            for ibin in range(len(ereco) - 1):
                ...
                data = evt_dict_sample['gamma'].query(energy_query).copy()
        ...
        cut_applicator = CutsApplicator(config=cfg, evt_dict=evt_dict_sample, outdir=outdir)
        cut_applicator.apply_cuts()

        # Irf Maker
        print('### Building IRF...')
        irf_maker = IrfMaker(config=cfg, evt_dict=evt_dict_sample, outdir=outdir)
        irf_maker.build_irf()
